...
export function rehypeHeaderNumbering() {
  return (tree) => {
    ...
    visit(tree, 'element', (node, index, parent) => {
      ...
      const isFootnoteLabel = node.properties?.id?.includes('footnote') || 
      ...
      const isFootnoteText = headerText === 'footnotes' || headerText === '각주';

      if (tagName === 'h2' && (isFootnoteLabel || isFootnoteText)) {
        // 1. HR 요소 생성
        const hr = { type: 'element', tagName: 'hr', properties: { className: ['footnote-sep'] }, children: [] };
